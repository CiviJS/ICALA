...
<body class="fade-in">
    ...
    <div class="layout-container wide">
        ...
        <header class="header-accent"
            style="border-left: 5px solid var(--primary-color); padding-left: 20px; margin-bottom: 30px;">
            <div
        ...
        <div style="gap: 10px; margin:1vh">
                    <form method="GET" action="/planillas/ver/{{ $data['planilla']->uuid }}" class="search-bar">
                        <input type="text" name="campoUsuario" id="campoUsuario" placeholder="Ingresa el campo a buscar..">
                        <button type="submit" class="btn btn-primary" style=" margin-top: 10px;" >Buscar Usuario</button>
                    </form>               
                     <a href="{{ url('/planillas/ver/'.  $data['planilla']->uuid)}}" class="btn btn-outline" style=" margin-top: 10px;">Mostrar Todo</a>
                </div>
                        ...
                        @foreach ($data['usuarios'] as $usuario)
                            <tr>
                                ...
                                <td data-label="Acción" style="text-align: center;">
                                    <form
                                        action="{{ url('/planilla/Asistencia/' . $data['planilla']->uuid . '/' . $usuario->uuid) }}"
